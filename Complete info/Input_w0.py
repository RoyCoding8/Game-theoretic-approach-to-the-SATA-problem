...
# Worker Location
df = pd.read_csv(r'.\Effect_of_workers\100_workers\worker_location.csv')
m=len(df)
...
df = pd.read_csv(r'.\Effect_of_workers\100_workers\worker_range.csv')
...
# Worker Skills
df = pd.read_csv(r'.\Effect_of_workers\100_workers\worker_skills.csv')
...
# Worker Cost
df = pd.read_csv(r'.\Effect_of_workers\100_workers\worker_cost.csv')
...
df = pd.read_csv(r'.\Effect_of_workers\100_workers\worker_task_history.csv')
task_history = [[] for _ in range(m+1)]
for _,row in df.iterrows():
    key=row['worker']
    value=row['task']
    task_history[key].append(value)
