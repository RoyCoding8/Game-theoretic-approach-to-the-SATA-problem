...
df = pd.read_csv(r'.\Effect_of_tasks\500_tasks\task_budget.csv')
n=len(df)
budget = [0.0]*(n+1)
...
# Task Location
df = pd.read_csv(r'.\Effect_of_tasks\500_tasks\task_location.csv')
...
df = pd.read_csv(r'.\Effect_of_tasks\500_tasks\task_skills.csv')
task_skills = [[]]*(n+1)
for _,row in df.iterrows():
    key=row['task']
    value=row['req_skill']
